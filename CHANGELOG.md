...
### Fixed & polished
- **Dark mode was broken** — the dark palette contained an invalid 10-digit colour (`#FF35356088`), so the whole theme dictionary failed to parse and switching to dark left the UI half-styled. Fixed the colour and hardened `ThemeManager` to load the new palette before dropping the old one (a malformed theme can no longer leave the app unthemed).
